...
class FedEBPTripletFaceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ebp_anc_img = Image.open(ebp_anc_img)
        ebp_pos_img = Image.open(ebp_pos_img)
        ebp_neg_img = Image.open(ebp_neg_img)
        anc_img.putalpha(Image.fromarray(np.array(ebp_anc_img)[:,:,1]))
        pos_img.putalpha(Image.fromarray(np.array(ebp_pos_img)[:,:,1]))
        neg_img.putalpha(Image.fromarray(np.array(ebp_neg_img)[:,:,1]))
